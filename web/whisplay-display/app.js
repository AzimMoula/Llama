...
function applyState(data) {
  ...
  currentDeviceStatus = String(data.status || "").toLowerCase();
  setPressed(currentDeviceStatus === "listening");
...
function sendButtonToggle() {
  if (!ws || ws.readyState !== WebSocket.OPEN) return;
  ws.send(JSON.stringify({ type: "button_toggle" }));
}
...
function sendButtonForCurrentState() {
  // Ignore accidental double taps from touch/click synthesis.
  const now = Date.now();
  if (now - lastButtonTapAt < 250) return;
  lastButtonTapAt = now;
  sendButtonToggle();
}

// Use click events only, with action chosen by server-reported state.
btn.addEventListener("click", (event) => {
  event.preventDefault();
  sendButtonForCurrentState();
});
...
function requestUserMedia(constraints) {
  ...
  if (!legacyGetUserMedia) {
    return Promise.reject(new Error("getUserMedia API is unavailable"));
  }
...
function sendRecordComplete() {
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({ type: "record_complete" }));
  }
}

async function ensureWebAudioRecorderRunning() {
  if (!shouldRecordAudio) return;
  ...
  try {
    ...
    if (!audioStream) {
      audioStream = await requestUserMedia({ audio: true });
    }
    ...
    mediaRecorder.onstop = () => {
      ...
      // If recording is still desired, this stop was unexpected. Auto-restart quickly.
      if (shouldRecordAudio) {
        setTimeout(() => {
          void ensureWebAudioRecorderRunning();
        }, 120);
        return;
      }
    ...
    // Emit data chunks frequently so short utterances are still captured.
...
async function startWebAudioRecording() {
  ...
  await ensureWebAudioRecorderRunning();
}
