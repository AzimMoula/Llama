...
def _detect_platform():
    ...
    # Detect from device-tree model first.
    try:
...
class MockGPIO:
    BCM = BOARD = OUT = IN = HIGH = LOW = BOTH = 0
    PUD_UP = PUD_DOWN = 0
    ...
    def setup(self, a, b, pull_up_down=None):
        pass
...
try:
    ...
    print(">>> Jetson.GPIO not found. Mocking GPIO.")
    GPIO = MockGPIO()
...
try:
    ...
    class _MockGpiod:
        LINE_REQ_DIR_OUT = 0
        LINE_REQ_DIR_IN = 1
        LINE_REQ_FLAG_BIAS_PULL_UP = 0

        @staticmethod
        def Chip(name):
            return _MockGpioChip(name)
...
class WhisplayBoard:
    ...
    def __init__(self):
        ...
        if self.platform == "rpi":
            ...
            print(f"[WhisplayBoard] Unsupported platform '{self.platform}', using mock mode.")
            self.platform = "mock"
            self._init_mock()
        ...
        if self.platform != "mock":
            ...
            print("[WhisplayBoard] Skipping physical LCD initialization for HDMI mode.")

    # ==================== Mock Initialization ====================
    def _init_mock(self):
        """Initialize no-op GPIO/SPI for unsupported platforms (e.g. Windows)."""
        ...
        try:
            ...
            # Some Linux environments provide spidev module but no /dev/spidev* device.
            # Fall back to an in-process no-op SPI object so UI can still run.
            class _NoopSpi:
                max_speed_hz = 0
                mode = 0
        ...
        # Tell the Jetson these pins are outputs before trying to PWM them
        self.red_pwm = MockPWM(self.RED_PIN, 100)
        self.green_pwm = MockPWM(self.GREEN_PIN, 100)
        self.blue_pwm = MockPWM(self.BLUE_PIN, 100)
        self.backlight_pwm = MockPWM(self.LED_PIN, 100)
        ...
        if self.backlight_mode:  # PWM mode
            if self.backlight_pwm is None:
                if self.platform == "rpi":
                    ...
                    # Use MockPWM instead of GPIO.PWM to avoid Pin 15 error
                    self.backlight_pwm = MockPWM(self.LED_PIN, 1000)
                
                if self.backlight_pwm:
                    self.backlight_pwm.start(100)
            
            if self.backlight_pwm and 0 <= brightness <= 100:
                duty_cycle = 100 - brightness
                self.backlight_pwm.ChangeDutyCycle(duty_cycle)
