...
export function getWavFileDurationMs(buffer: Buffer): number {
  // WAV 文件头部信息在前 44 字节
...
export const getRecordFileDurationMs = async (
  ...
  try {
    ...
    const isWebm =
      ...
      data[3] === 0xa3;
    if (isWebm) {
      // No native WebM duration parser here; treat as non-empty recording.
    ...
    // Unknown format: if non-empty, allow ASR attempt.
    return data.length > 0 ? 1000 : 0;
  } catch (error) {
    return 0;
  }
