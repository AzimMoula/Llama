...
const shouldRejectWakePromptText = (value: string): { reject: boolean; reason: string } => {
  const normalized = normalizeWakePromptText(value);
  if (!normalized) {
    return { reject: true, reason: "empty" };
  }

  const minChars = Math.max(
    1,
    parseInt(process.env.WAKE_WORD_POST_ASR_MIN_CHARS || "4", 10),
  );
  const minWords = Math.max(
    1,
    parseInt(process.env.WAKE_WORD_POST_ASR_MIN_WORDS || "1", 10),
  );

  const words = normalized.split(" ").filter(Boolean);
  if (normalized.length < minChars) {
    return { reject: true, reason: `too_short_chars(${normalized.length}<${minChars})` };
  }
  if (words.length < minWords) {
    return { reject: true, reason: `too_short_words(${words.length}<${minWords})` };
  }

  const defaultRejectSet = [
    "you",
    "on",
    "uh",
    "um",
  ...
  const customRejectSet = String(process.env.WAKE_WORD_POST_ASR_REJECT_EXACT || "")
    .split(",")
    .map((item) => normalizeWakePromptText(item))
    .filter(Boolean);
  const rejectSet = new Set([...defaultRejectSet, ...customRejectSet]);
  if (rejectSet.has(normalized)) {
    return { reject: true, reason: `filtered(${normalized})` };
  }
...
export const flowStates: Record<FlowName, FlowStateHandler> = {
  ...
  listening: (ctx: ChatFlowContext) => {
    ...
    const stopListeningCapture = (reason: string) => {
      if (stopIssued) return;
      stopIssued = true;
      console.log(`[Listening] Stopping manual recording (${reason})`);
      stop();
      display({
        RGB: "#ff6800",
        image: "",
    ...
    const forcedStopTimer = setTimeout(() => {
      ...
      stopListeningCapture("max_manual_timeout");
    }, maxManualListenMs);
    ...
    onButtonReleased(() => {
      const elapsed = Date.now() - listenStartedAt;
      if (elapsed < minManualRecordMs) {
        const waitMs = minManualRecordMs - elapsed;
        console.log(
          `[WebAudio] Early release at ${elapsed}ms; scheduling stop in ${waitMs}ms (min ${minManualRecordMs}ms)`,
        );
        setTimeout(() => {
          if (ctx.currentFlowName !== "listening") return;
          stopListeningCapture("min_manual_duration_reached");
        }, waitMs);
        return;
      }
      stopListeningCapture("button_release");
    });
    result
      .then(() => {
        clearForcedStopTimer();
  ...
  wake_listening: (ctx: ChatFlowContext) => {
    ...
    display({
      ...
      emoji: DEFAULT_EMOJI,
      RGB: "#00ff00",
      text: "Wake word detected. Preparing mic...",
      rag_icon_visible: false,
    });
    ...
    const startWakeCapture = (retryCount: number = 0) => {
      display({
        ...
        emoji: DEFAULT_EMOJI,
        RGB: "#00ff00",
        text: `Listening for ${wakeListenSec}s...`,
        rag_icon_visible: false,
      });

      recordAudioForDuration(ctx.currentRecordFilePath, wakeListenSec)
        .then(() => {
          ctx.transitionTo("asr");
        })
        .catch((err) => {
  ...
  asr: (ctx: ChatFlowContext) => {
    ...
    Promise.race([
      ...
      if (result) {
        if (ctx.wakeSessionActive) {
          const wakePromptCheck = shouldRejectWakePromptText(result);
          if (wakePromptCheck.reject) {
            console.log(
              `[Wake] Ignoring low-confidence wake prompt: ${wakePromptCheck.reason} text="${normalizeWakePromptText(result)}"`,
            );
            result = "";
          }
        }
      ...
      if (ctx.wakeSessionActive) {
        ...
        if (ctx.wakeEmptyAsrRetries < emptyRetryLimit && ctx.shouldContinueWakeSession()) {
          ...
          console.log(
            `[Wake] Empty ASR after wake; retrying capture (${ctx.wakeEmptyAsrRetries}/${emptyRetryLimit}).`,
          );
          ctx.transitionTo("wake_listening");
        } else if (continueOnEmptyWakeAsr && ctx.shouldContinueWakeSession()) {
          ctx.transitionTo("wake_listening");
        } else {
          if (!continueOnEmptyWakeAsr) {
            console.log("[Wake] Empty ASR after wake; ending wake session to avoid loop.");
          }
          ctx.endWakeSession();
          ctx.transitionTo("sleep");
        }
      ...
      const isTimeout = errText.includes("asr timeout") || errText.includes("timed out");
      display({
        status: "error",
        emoji: "⚠️",
        text: isTimeout
          ? "Speech recognition timed out. Please speak clearly and try again."
  ...
  answer: (ctx: ChatFlowContext) => {
    ...
    const continueWakeSessionAfterAnswer =
      String(process.env.WAKE_WORD_CONTINUE_SESSION_AFTER_ANSWER || "false").toLowerCase() === "true";

    // Always arm answer-exit flow before any early-return reply path.
    getPlayEndPromise().then(() => {
      if (ctx.currentFlowName === "answer") {
        clearPendingCapturedImgForChat();
        display({ image_icon_visible: false });
        if (ctx.wakeSessionActive || ctx.endAfterAnswer) {
          if (ctx.endAfterAnswer || !continueWakeSessionAfterAnswer) {
            ctx.endWakeSession();
            ctx.transitionTo("sleep");
          } else {
            ctx.transitionTo("wake_listening");
