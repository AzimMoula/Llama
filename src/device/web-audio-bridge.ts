...
class WebAudioBridge {
  ...
  private finishRecording(): void {
    ...
    const cleanupAndResolve = (converted: boolean) => {
      ...
      if (!converted) {
        // Fallback: persist browser-native WebM and return that path.
        const fallbackWebmPath = rec.outputPath.replace(/\.(wav|mp3)$/i, ".webm");
        try {
          fs.writeFileSync(fallbackWebmPath, allData);
          rec.resolve(fallbackWebmPath);
          return;
        } catch {}
        // Last resort: write bytes to the requested path.
        try {
          fs.writeFileSync(rec.outputPath, allData);
        } catch {}
      }
  ...
  handleRecordComplete(): void {
    ...
    // Ignore unexpected recorder stops (browser quirks / transient permission issues)
    // unless the server has explicitly requested stop.
    if (!this.recording.stopRequested) {
      console.warn("[WebAudio] Ignoring unsolicited record_complete");
      return;
    }
