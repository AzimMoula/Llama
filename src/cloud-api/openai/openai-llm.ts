...
const openaiMaxTokens = parseInt(process.env.OPENAI_MAX_TOKENS || "128", 10);
const openaiTemperature = parseFloat(process.env.OPENAI_TEMPERATURE || "0.15");
const openaiTopP = parseFloat(process.env.OPENAI_TOP_P || "0.9");
...
const chatWithLLMStream: ChatWithLLMStreamFunction = async (
  ...
  if (openaiUseStream) {
    ...
    try {
      ...
      // Common with unstable local OpenAI-compatible streams (llama-cpp server).
      console.log("OpenAI stream interrupted:", error?.message || error);
      if (!partialAnswer) {
        partialAnswer =
          "I had a temporary model stream issue. Please try your request again.";
        partialCallback(partialAnswer);
      }
    }
